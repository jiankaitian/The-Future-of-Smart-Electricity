...
if __name__ == "__main__":
    ...
    for round in range(conf['number_of_communications']):
        ...
        # 在这里初始化 client_updates
        client_updates = []  # 初始化客户端更新列表

        for client in tqdm(clients, desc="Collecting client updates"):
            client.train(epochs=conf['local_epochs'], batch_size=conf['batch_size'], loss_fn=nn.MSELoss(),
                         optimizer=optimizer)
        ...
        with torch.no_grad():
            ...
            for client in clients:
                ...
                for data, targets in test_loader:
                    ...
                    print("Data batch shape before unsqueeze:", data.shape)
                    print("Data batch example:", data[0])
                    print("Target batch example:", targets[0])
                    if data.dim() == 2:
                        data = data.unsqueeze(1)  # 将data变为[batch_size, 1, features]
                    print("Data batch shape before unsqueeze11:", data.shape)
                    print("Data batch example11:", data[0])
                    print("Target batch example11:", targets[0])
                    outputs = model(data)
